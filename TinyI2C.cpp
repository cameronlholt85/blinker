...
uint8_t TinyI2C_::transfer(uint8_t data)
{
...
uint8_t TinyI2C_::read(void)
{
  ...
  uint8_t data = TinyI2C_::transfer(USISR_8bit);
  ...
  if (I2Ccount == 0)
    ...
    USIDR = 0x00;
  TinyI2C_::transfer(USISR_1bit); // Generate ACK/NACK.

  return data; // Read successfully completed
}

uint8_t TinyI2C_::readLast(void)
{
  I2Ccount = 0;
  return TinyI2C_::read();
}

bool TinyI2C_::write(uint8_t data)
{
  /* Write a byte */
  PORT_USI_CL &= ~(1 << PIN_USI_SCL); // Pull SCL LOW.
  USIDR = data;                       // Setup data.
  TinyI2C_::transfer(USISR_8bit);     // Send 8 bits on bus.

  /* Clock and verify (N)ACK from slave */
  DDR_USI &= ~(1 << PIN_USI_SDA); // Enable SDA as input.
  if (TinyI2C_::transfer(USISR_1bit) & 1 << TWI_NACK_BIT)
    return false;
...
bool TinyI2C_::start(uint8_t address, int readcount)
{
  if (readcount != 0)
  {
    I2Ccount = readcount;
...
bool TinyI2C_::restart(uint8_t address, int readcount)
{
  return TinyI2C_::start(address, readcount);
}

void TinyI2C_::stop(void)
{
...
TinyI2C_ TinyI2C = TinyI2C_(); // Instantiate a TinyI2C object
